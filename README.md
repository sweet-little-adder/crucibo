...
**US equities trading infrastructure** — ingest, backtest, paper, and live on one path (crypto optional).

Ingest from **Alpha Vantage** (free tier) → event-driven backtest with fees/slippage → paper and `live-equities` with kill switches, order state, and measured latency.
...
| Variable | Used by |
|----------|---------|
| `ALPHA_VANTAGE_API_KEY` | `alphavantage-daily`, `alphavantage-intraday`, `live-equities` |
| `POLYGON_API_KEY` | `polygon-trades` |
| `ALPACA_API_KEY` / `ALPACA_API_SECRET` | `live-equities --broker alpaca_paper` |
...
| [docs/VISION.md](docs/VISION.md) | Thesis & outcomes |
