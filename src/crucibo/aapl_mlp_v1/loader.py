"""Load aapl_mlp_v1 artifact bundles (manifest.json + weights.npz)."""
...
def resolve_artifact_dir(path: Path) -> Path:
    ...
    raise ValueError(f"aapl_mlp_v1 artifact must be a directory or weights.npz: {path}")


def load_aapl_mlp_v1_model(path: Path) -> AaplMlpV1Model:
    ...
    if int(manifest.get("contract_version", -1)) != CONTRACT_VERSION:
        raise ValueError(f"unsupported aapl_mlp_v1 contract_version in {manifest_path}")
    if manifest.get("model_type") != MODEL_TYPE_MLP_V1:
        raise ValueError(f"unsupported aapl_mlp_v1 model_type: {manifest.get('model_type')!r}")
    if int(manifest.get("feature_schema_version", -1)) != FEATURE_SCHEMA_VERSION:
        raise ValueError("unsupported aapl_mlp_v1 feature_schema_version")
    names = manifest.get("feature_names")
    if tuple(names) != FEATURE_NAMES:
        raise ValueError("aapl_mlp_v1 feature_names mismatch with crucibo feature schema v1")

    with np.load(weights_file, allow_pickle=False) as data:
        return AaplMlpV1Model(
            w1=data["w1"],
            b1=data["b1"],
            w2=data["w2"],
            b2=data["b2"],
