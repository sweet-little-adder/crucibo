...
def resolve_strategy(
    ...
    *,
    ...
    if key in {"aapl_mlp_v1", "aaplmlpv1", "morning_star", "morningstar", "external"}:
        if model_path is None:
            raise ValueError(
                "aapl_mlp_v1 strategy requires --model PATH to an artifact directory "
                "(manifest.json + weights.npz)"
            )
        return AaplMlpV1Strategy.from_artifact(model_path)
    raise ValueError(f"unknown strategy: {name!r} (try flat | buy_hold | neural | aapl_mlp_v1)")
